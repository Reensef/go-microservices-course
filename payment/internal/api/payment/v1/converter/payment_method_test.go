...
func TestProtoPaymentMethodToModel(t *testing.T) {
	tests := []struct {
		name                string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			modelMethod := ProtoPaymentMethodToModel(tt.paymentMethod)
			if modelMethod != tt.expectedModelMethod {
				t.Errorf("ProtoPaymentMethodToModel() = %v, want %v", modelMethod, tt.expectedModelMethod)
			}
		})
	}
}

func TestModelPaymentMethodToProto(t *testing.T) {
	tests := []struct {
		name                string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			protoMethod := ModelPaymentMethodToProto(tt.paymentMethod)
			if protoMethod != tt.expectedProtoMethod {
				t.Errorf("ModelPaymentMethodToProto() = %v, want %v", protoMethod, tt.expectedProtoMethod)
			}
		})
	}
}
