...
// через Envoy-гейтвей — сам он ничего не поднимает и не останавливает, см.
// `task compose:*:up` и `task compose:envoy:up`. Адрес гейтвея —
// http://localhost:8080 по умолчанию, переопределяется через
// APITEST_GATEWAY_BASE_URL (см. internal/env).
//
// Шаги:
