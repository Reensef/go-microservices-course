...
func TestModelPartToProto(t *testing.T) {
	part := &model.Part{
		Uuid:      uuid.New(),
		CreatedAt: time.Now(),
		UpdatedAt: time.Now(),
	...
	protoPart := ModelPartToProto(part)
	...
	assert.Equal(t, ModelPartCategoryToProto(part.Info.Category), protoPart.Category)
	...
	for key, protoMeta := range protoPart.Metadata {
		...
		assert.Equal(t, MultiValueToProto(&meta), protoMeta)
	}
...
func TestProtoPartToModel(t *testing.T) {
	...
	modelPart, err := ProtoPartToModel(protoPart)
	require.Nil(t, err)
	...
	assert.Equal(t, modelPart.Info.Category, ProtoPartCategoryToModel(protoPart.Category))
	...
	for key, protoMeta := range protoPart.Metadata {
		...
		assert.Equal(t, &modelMeta, ProtoValueToMultiValue(protoMeta))
	}
...
func TestProtoPartToModel_InvalidUuid(t *testing.T) {
	protoPart := &inventoryV1.Part{
		Uuid: "",
	}
