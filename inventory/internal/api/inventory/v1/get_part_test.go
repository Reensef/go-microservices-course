...
func TestGetPart_ValidUuid(t *testing.T) {
	...
	service.EXPECT().GetPartByUuid(t.Context(), uuid).Return(&model.Part{}, nil).Once()

	a := NewAPI(service)

	response, err := a.GetPart(t.Context(), &inventoryV1.GetPartRequest{Uuid: uuid.String()})
	assert.NoError(t, err)
	assert.NotNil(t, response)
	assert.NotNil(t, response.Part)
}

func TestGetPart_InvalidUuid(t *testing.T) {
	service := mocks.NewMockInventoryService(t)

	a := NewAPI(service)

	response, err := a.GetPart(t.Context(), &inventoryV1.GetPartRequest{Uuid: "invalid-uuid"})
...
func TestGetPart_ServiceError(t *testing.T) {
	...
	service.EXPECT().GetPartByUuid(t.Context(), uuid).Return(nil, fmt.Errorf("error")).Once()
